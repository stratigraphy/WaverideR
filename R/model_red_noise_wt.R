...
#'@examples
#'\dontrun{
#'#'#generate average spectral power curves based on red noise curves which are\cr
#'# based on the magnetic susceptibility record of the Sullivan core of De pas et al., (2018)
#'
...
dj <- wavelet$dj
 ...
 dat <- as.matrix(data)
  ...
  red_noise <- foreach (j=1:n_simulations, .combine='cbind',.options.snow = opts)%dopar% {
    #phi_data_2 <- truncnorm::rtruncnorm(n=1,a=1/100000,b=1-(1/100000),mean=phi_data,sd=0.5) #old option
    x <- (colorednoise::colored_noise(timesteps = timesteps_data, mean=mean_data,
                                      sd=sd_data, phi_data))
    ...
    Power.avg = rowMeans(as.matrix(Mod(Wave)^2 / matrix(rep(scales, series.length), nrow=scales.length)))
