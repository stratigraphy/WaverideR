...
wavelet_uncertainty <- function(tracked_cycle=NULL,
  ...
  if (verbose==TRUE){
    print(paste0("total duration is: ", time[nrow(time),2]))
    print(paste0("uncertainty is: ", time_plus[nrow(time_plus),2]-time[nrow(time),2]))
    print(paste0("uncertainty is defined as ",multi," standard deviations "))
    print(paste0("uncertainty defined as standard deviation is: ",((a/k)/b)," cycle per cycle"))
  ...
  if (genplot_time==TRUE){
    ...
    plot(time_min,type="l",col="red",ylim=c(0,time_plus[nrow(time_plus),2]))
  ...
  if (genplot_uncertainty==TRUE){
    ...
    plot(data[,1],1/data$f0,type="l",ylim=c(min_y,max_y),lwd=2,xlab="depth",ylab="period (m)")
    lines(data[,1],1/(data$f0-data$sd_morlet*multi
    ),col="red",lwd=2)
    lines(data[,1],1/(data$f0+data$sd_morlet*multi
    ),col="blue",lwd=2)
  }
  ...
  if (genplot_uncertainty_wt==TRUE){
    ...
    lines(data[,1],log2(1/(data$f0-data$sd_morlet*multi
    )),col="red",lwd=2)
    lines(data[,1],log2(1/(data$f0+data$sd_morlet*multi
    )),col="blue",lwd=2)

    combined_sedrate <-cbind(data[,1],data$f0,data$sd_morlet*multi)
  ...
  colnames(data) <- c("depth","period","frequency",
                       "uncertainty in frequency FWHM",
                      "uncertainty in frequency x_times SD","time mean",
                      "time plus x_times sd","time min x_times sd")
