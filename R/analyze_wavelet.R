...
#' @param omega_nr Number of cycles contained within the wavelet.
#'
#' @return
#' The output is a list (wavelet object) with the results of the continuous wavelet transform (CWT).
#'
#' @author
#' Code based on the R package \link[WaveletComp]{WaveletComp-package} and \link[biwavelet]{biwavelet-package}
#' which are based on the wavelet MATLAB code written by Christopher Torrence and Gibert P. Compo.
#'
#' @references
...
#'
#'
#'#Example 2. Using the magnetic susceptibility data set of De pas et al., (2018)
#'mag_wt <-
#'analyze_wavelet(
#'data = mag,
#'dj = 1/100,
...
analyze_wavelet <-
  function(data=NULL,
    ...
    if (verbose == TRUE){
    print(paste0("dataset interpolated to: ", round(dt,10)))}
    ...
    axis.2 = log2(periods)
    fourier.factor = (2*pi)/omega0
    coi = fourier.factor * sqrt(2) * dt * c(1E-5, 1:((series.length+1)/2-1), rev((1:(series.length/2-1))), 1E-5)
    coi.x = c(axis.1[c(1, 1)] - dt*0.5, axis.1, axis.1[c(series.length, series.length)] + dt*0.5)
    logyint = axis.2[2] - axis.2[1]
    yl = c(log2(periods[scales.length]) + 0.5*logyint, log2(periods[1]) - 0.5*logyint)
    yr = rev(yl)
    coi.y = c(yl, log2(coi), yr)
    x <- dat[,1]
    y <- dat[,2]

    output <- list(Wave = Wave, Phase = Phase, Ampl = Ampl, Power = Power,dt=dt,dj=dj,
                   Power.avg = Power.avg,Period = periods, Scale = scales, coi.1 = coi.x, coi.2 = coi.y,
                   nc = series.length, nr = scales.length, axis.1 = axis.1, axis.2 = axis.2,omega_nr=omega_nr,
                   x=x,y=y
    )
