...
#'@param time_dir The direction of the proxy record which was assumed during achoring if time increases with increasing depth/time values
...
if (time_dir == TRUE){
  ...
  segments(x0=anchor_points[,1],
           y0=rep(min(proxy_signal[,2])*0.2,times=(nrow(anchor_points))),
           x1 = anchor_points[,1],
           y1 =anchor_points[,3],
           ylim= c(min(proxy_signal[,2]),max(proxy_signal[,2])),
           xlim = (c(
  ...
  points(x=anchor_points[,1],y=anchor_points[,3],col="green",pch=19,
         ylim= c(min(proxy_signal[,2]),max(proxy_signal[,2])),
         xlim = (c(
           min(proxy_signal[, 1]), max(proxy_signal[, 1])
         )),cex=2)
  ...
  segments(x0=anchor_points[,2], y0=anchor_points[,4], x1 = anchor_points[,2],
           y1 = rep(max(astro_solution[,2])*1.2,times=(nrow(anchor_points))),
           col = "black", lwd = 2)
  points(x=anchor_points[,2],y=anchor_points[,4],col="red",pch=19,cex=2)
  ...
  fact <- (anchor_points[,1]-min(proxy_signal[, 1]))/
    (max(proxy_signal[, 1])-min(proxy_signal[, 1]))
  x1_vals <- ((max(astro_solution[, 1])-min(astro_solution[, 1]))*(fact))+min(astro_solution[, 1])


  segments(x0=anchor_points[,2],
           y0=rep(y0_val,times=(nrow(anchor_points)))
           ,x1 = x1_vals,
           y1 = rep(y1_val,times=(nrow(anchor_points))),
